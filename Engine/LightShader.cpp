...
bool LightShader::Render(ID3D11DeviceContext* ctxt, int indexCount, XMMATRIX world, XMMATRIX view, XMMATRIX project,XMMATRIX object, ID3D11ShaderResourceView* texture,XMFLOAT3 lightDir,XMFLOAT4 diffuseColor)
{
    bool result;
    result = SetShaderParams(ctxt, world, view, project,object, texture,lightDir,diffuseColor);
    if (!result)
        return false;
...
bool LightShader::InitShader(ID3D11Device* device, HWND hwnd, WCHAR* vsFilename, WCHAR* psFilename)
{
    ...
    D3D11_BUFFER_DESC matBuffDesc,lighBuffDesc;
...
bool LightShader::SetShaderParams(ID3D11DeviceContext* ctxt, XMMATRIX world, XMMATRIX view, XMMATRIX proj,XMMATRIX object, ID3D11ShaderResourceView* texture, XMFLOAT3 lightDir, XMFLOAT4 diffuseColor)
{
    HRESULT result;

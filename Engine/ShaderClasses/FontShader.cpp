...
bool FontShader::Init(ID3D11Device* device, HWND hwnd)
{
	...
	result = InitShader(device, hwnd, (WCHAR*)L"../Engine/Shaders/FontVS.hlsl", (WCHAR*)L"../Engine/Shaders/Font.hlsl");
	return result;
}
...
bool FontShader::InitShader(ID3D11Device* device, HWND hwnd, WCHAR* vsFilename, WCHAR* psFilename)
{
	...
	D3D11_BUFFER_DESC matBuffDesc,pixelBuffDesc;
	...
	result = D3DCompileFromFile(vsFilename, NULL, NULL, "FontVertexShader", "vs_5_0", D3D10_SHADER_ENABLE_STRICTNESS, 0, &vertexshaderBuff, &errorMsg);
	if (FAILED(result))
	{
		if (errorMsg)
	...
	result = D3DCompileFromFile(psFilename, NULL, NULL, "FontPixelShader", "ps_5_0", D3D10_SHADER_ENABLE_STRICTNESS, 0, &pixelShaderBuff, &errorMsg);
	if (FAILED(result))
	{
		if (errorMsg)
	...
	pixelBuffDesc.Usage = D3D11_USAGE_DYNAMIC;
	pixelBuffDesc.ByteWidth = sizeof(Pixelbuffer);
	pixelBuffDesc.BindFlags = D3D11_BIND_CONSTANT_BUFFER;
	pixelBuffDesc.CPUAccessFlags = D3D11_CPU_ACCESS_WRITE;
	pixelBuffDesc.MiscFlags = 0;
	pixelBuffDesc.StructureByteStride = 0;

	result = device->CreateBuffer(&matBuffDesc, NULL, &m_pixelBuffer);
	if (FAILED(result))
		return false;
...
void FontShader::OutputShaderErrorMsg(ID3D10Blob* msg, HWND hwnd, WCHAR* file)
{
	char* compileErrors;
	...
	fout.open("FontShaderError.error");
	...
	MessageBox(hwnd, L"Error compiling shader.  Check FontShaderError.error for message.", file, MB_OK);
	return;
}
...
bool FontShader::SetShaderParams(ID3D11DeviceContext* ctxt, XMMATRIX world, XMMATRIX view, XMMATRIX proj, ID3D11ShaderResourceView* texture, XMFLOAT4 pixelColor)
{
	...
	result = ctxt->Map(m_pixelBuffer, 0, D3D11_MAP_WRITE_DISCARD, 0, &mappedSubresource);
	if (FAILED(result))
		return false;
	dataPtr1 = (Pixelbuffer*)mappedSubresource.pData;
	dataPtr1->pixelColor = pixelColor;
	ctxt->Unmap(m_pixelBuffer, 0);
	ctxt->PSSetConstantBuffers(bufferNum, 1, &m_pixelBuffer);


	return true;
}

void FontShader::RenderShader(ID3D11DeviceContext* ctxt, int indexCount)
{
